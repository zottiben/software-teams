...
Spawn the committer natively by name. Claude Code loads the spec from `.claude/agents/software-teams-committer.md` (generated by `$ST_CLI sync-agents` — resolve per `commands/_shared/cli-invocation.md` — from `framework/agents/software-teams-committer.md`):
