...
export interface ConvertAgentsOptions {
  /**
   * Source directory containing canonical `jdi-*.md` agent specs.
   * Defaults to `.jdi/framework/agents` if it exists, else `framework/agents`.
   */
  sourceDir?: string;
  /**
...
function renderAgentOutput(parsed: ParsedAgentFile, sourcePath: string): string {
  ...
  const banner = `<!-- AUTO-GENERATED by jdi sync-agents — edit framework/agents/${fm.name}.md and re-run -->`;
  const footer = `JDI source: ${sourcePath}`;
 ...
 * (`.jdi/framework/agents`), fall back to repo-root `framework/agents`.
 */
function resolveDefaultSourceDir(cwd: string): string {
  const installed = join(cwd, ".jdi", "framework", "agents");
 ...
 * layout (`.jdi/framework/templates/RULES.md`), fall back to repo-root
 * `framework/templates/RULES.md`.
 */
function resolveDefaultRulesSource(cwd: string): string {
  const installed = join(cwd, ".jdi", "framework", "templates", "RULES.md");
  if (existsSync(installed)) return installed;
  return join(cwd, "framework", "templates", "RULES.md");
}

const CATALOGUE_BANNER =
  "<!-- AUTO-GENERATED by jdi sync-agents — edit framework/agents/*.md and framework/templates/AGENTS.md.template (if used) and re-run -->";
const RULES_BANNER =
  "<!-- AUTO-GENERATED by jdi sync-agents — edit framework/templates/RULES.md and re-run -->";
...
/**
 * Convert every canonical `jdi-*.md` spec under `sourceDir` to a
...
export async function convertAgents(
  ...
  const glob = new Bun.Glob("jdi-*.md");
