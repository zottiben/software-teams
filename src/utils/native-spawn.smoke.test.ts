 ...
 *   1. Run `bun run dev sync-agents` (or `jdi sync-agents`) in this repo so
 *      `.claude/agents/` is populated with the 24 native specs.
 *   2. From a Claude Code session in this repo, dispatch a Task call with
 *      `subagent_type="jdi-status"` and `prompt="echo OK"` (or any registered
 *      JDI subagent + a sentinel prompt).
 *   3. Assert the response contains the sentinel phrase ("OK") AND that the
 *      response embodies the agent's spec body (no "You are jdi-X. Read ..."
 *      preamble was needed — Claude Code resolved the spec from
 *      `.claude/agents/jdi-status.md` natively).
 *
 * CI behaviour:
 *   - Skipped because no Claude Code session is available to the test runner.
 *   - Set JDI_NATIVE_SPAWN_SMOKE=1 to opt in (still currently a no-op
...
const SMOKE_ENABLED = process.env.JDI_NATIVE_SPAWN_SMOKE === "1";

describe("native-spawn round-trip smoke (env-gated)", () => {
  test.skipIf(!SMOKE_ENABLED)(
    "Task subagent_type=\"jdi-status\" prompt=\"echo OK\" returns a response containing 'OK' (manual)",
    () => {
  ...
  test("smoke procedure is documented (always-runs sanity check)", () => {
    ...
    // documentation header drifts (e.g. someone renames `jdi-status` away
