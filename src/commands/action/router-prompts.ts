...
function buildPlanBrief(ctx: ActionContext, flow: { kind: "plan"; isRefinement?: boolean; isApproval?: boolean }): string[] {
  ...
  return [
    `## Plan Task`,
    `Produce the canonical Software Teams plan in \`.software-teams/plans/\`. Apply your Tier Decision Rule — three-tier is the DEFAULT for any non-trivial plan; single-tier is acceptable only when the plan is genuinely tiny (≤3 tasks AND single-team).`,
    ``,
    `### Tier outputs`,
    ``,
    `- **three-tier** (DEFAULT):`,
    `  - \`{slug}.spec.md\` — requirements + acceptance criteria`,
    `  - \`{slug}.orchestration.md\` — frontmatter MUST include \`available_agents:\`, \`primary_agent:\`, \`task_files:\`, \`issue: ${ctx.issueNumber}\`, \`repo: ${ctx.repo}\`; body contains the Tasks manifest table`,
    `  - Per-agent slices \`{slug}.T{n}.md\` — each MUST have \`agent:\`, \`tier: per-agent\`, \`spec_link:\`, \`orchestration_link:\` in frontmatter; one slice per task`,
    `- **single-tier** (only on Tier-Decision-Rule downgrade):`,
    `  - \`{slug}.plan.md\` — frontmatter \`task_files:\`, \`issue: ${ctx.issueNumber}\`, \`repo: ${ctx.repo}\`; manifest table only`,
    `  - Per-task \`{slug}.T{n}.md\` files`,
    ``,
    `### Scope rules`,
    ``,
    ...
    ``,
    `Begin with EXACTLY this line (replace \`{tier}\` with \`three-tier\` or \`single-tier\` per your choice):`,
    ``,
    `\`software-teams-planner\` has produced a {tier} plan for issue #${ctx.issueNumber}.`,
    ``,
    ...
    ``,
    `### Files written`,
    `- SPEC: \`{path}\` (three-tier only)`,
    `- ORCHESTRATION: \`{path}\` (three-tier only)`,
    `- Plan index: \`{path}\` (single-tier only)`,
    `- Task slices: \`{path1}\`, \`{path2}\`, ...`,
    ``,
    `### Verification`,
    `- [ ] {check 1}`,
    `- [ ] {check 2}`,
