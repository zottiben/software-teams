...
function buildPrePlanDiscoveryBrief(ctx: ActionContext): string[] {
  return [
    ...
    `- Budget: at most ~20 file reads + a handful of \`Glob\` / \`Grep\` passes on a fresh issue. On a conversational pass (i.e. you have a previous-comment answer to produce), raise the budget to ~35 reads + as many \`git log\` / \`git blame\` / grep passes as needed to actually answer the user's question — a thin answer with no evidence is worse than no answer.`,
    `- \`Bash\` is allowed only for read-only inspection (\`git log\`, \`git diff\`, \`git blame\`, \`ls\`, \`cat\`).`,
    `- Keep your final response ≤ 80 lines on a fresh issue, ≤ 150 lines on a conversational pass (answers eat budget — that's fine, that's what they're for).`,
    ``,
    `## Response Format (MANDATORY)`,
    ``,
    `Begin with EXACTLY this line:`,
