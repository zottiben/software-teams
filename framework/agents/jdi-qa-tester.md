...
Execute a planned test task generated by jdi-planner. This mode writes actual test files and runs them.
...
4. Write test files using the detected test framework's syntax (bun:test → `describe/test/expect`, vitest → `describe/it/expect`, jest → `describe/it/expect`, etc.)
