...
Canonical scaffolding the JDI planner and implement-plan skills read and write
against. All placeholders use `{like_this}` syntax.

## Planning Templates

JDI supports two plan shapes. Pick based on plan size and team shape, not by
preference.
...
Slug derivation rules live in `framework/agents/jdi-planner.md` (lowercase,
...
**Use single-tier when:** `/jdi:quick` flows, hotfixes, or plans with 1-3
...
| `SUMMARY.md` | Per-plan outcome record (written by jdi-programmer post-execution) |
