...
The full list of registered specialists and the orchestration / quality rules for this project are imported below. Both files are auto-generated by `jdi sync-agents` from `framework/agents/` and `framework/templates/RULES.md` — do **not** hand-edit them; re-run `jdi sync-agents` after changing the source.
