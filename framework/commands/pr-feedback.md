...
Spawn the specialist natively by name. Claude Code loads the spec from `.claude/agents/jdi-pr-feedback.md` (generated by `jdi sync-agents` from `framework/agents/jdi-pr-feedback.md`):

```
Agent(
  subagent_type="jdi-pr-feedback",
  mode="acceptEdits",
