...
Spawn one specialist agent directly via Task tool, **natively by name**.
...
- `source: jdi` → check `.claude/agents/{name}.md` (generated by `jdi sync-agents`
  from `.jdi/framework/agents/{name}.md` or `framework/agents/{name}.md` in the
  self-hosting JDI repo)
...
prompt. JDI specialists are placed there by `jdi sync-agents`, which converts
the canonical specs in `framework/agents/jdi-*.md` to the Claude Code format.
...
jdi-backend / jdi-frontend spec inside the prompt as a last resort. See
