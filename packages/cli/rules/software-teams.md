...
Band-aids are forbidden: silenced types (`as any`, `@ts-ignore`, `// eslint-disable` to dodge a real error), swallowed exceptions, duplicated logic you could refactor, `// TODO` placeholders, or deleting/skipping a test to make the suite green. If only a workaround fits the task scope, **STOP and escalate** to the orchestrator with the correct fix described — do not ship the shortcut silently. A quick fix that creates tech debt is a worse outcome than an honest escalation.
