...
describe("sync-agents — convertAgents flow", () => {
  test("default run writes 34 agents + AGENTS.md + RULES.md", async () => {
...
describe("sync-agents — command flow", () => {
  test("equivalent to `software-teams sync-agents --dry-run`: exits 0, writes nothing", async () => {
    ...
    expect(out.written).toBe(36); // 34 agents + AGENTS.md + RULES.md (planned, not written)
    expect(existsSync(join(cwd, ".claude"))).toBe(false);
  });

  test("equivalent to `software-teams sync-agents`: exits 0, writes 34 + AGENTS.md + RULES.md", async () => {
    const cwd = await makeFixtureCwd();
