...
2. **Create Team** — \`TeamCreate(team_name: "{slug}-team")\` where \`{slug}\` is the \`current_plan.slug\` field from \`state.yaml\` (set by \`software-teams-planner\` at plan-write time). The literal \`{slug}-team\` pattern is the convention shared by both single-tier (§8) and three-tier (§3T.8) execution paths in \`skills/implement-plan/SKILL.md\` — do not invent a different team-name format.
