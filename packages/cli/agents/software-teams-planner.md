...
## Respect Pre-Answered Questions
...
**Do NOT manually edit `.software-teams/state.yaml`** — state transitions are handled via CLI commands (e.g. `$ST_CLI state plan-ready` — resolve the CLI per `.claude/skills/st-support/cli-invocation.md` (CLI) or `${CLAUDE_PLUGIN_ROOT}/skills/st-support/cli-invocation.md` (plugin)).
...
1. Read scaffolding via the targeted CLIs (preferred — returns slices, not whole files). Resolve the CLI per `.claude/skills/st-support/cli-invocation.md` (CLI) or `${CLAUDE_PLUGIN_ROOT}/skills/st-support/cli-invocation.md` (plugin), then:
...
**Do NOT manually edit `.software-teams/state.yaml`** — use `$ST_CLI state` CLI commands for transitions (resolve the CLI per `.claude/skills/st-support/cli-invocation.md` (CLI) or `${CLAUDE_PLUGIN_ROOT}/skills/st-support/cli-invocation.md` (plugin)). Only record decisions, deviations, or blockers via `@ST:StateUpdate`.
...
Use the `$ST_CLI roadmap` CLI (resolve per `.claude/skills/st-support/cli-invocation.md` (CLI) or `${CLAUDE_PLUGIN_ROOT}/skills/st-support/cli-invocation.md` (plugin)) to add the plan entry. **Do NOT edit `.software-teams/roadmap.yaml` with Edit/Write** — the CLI is faster, deterministic, and avoids burning tokens on YAML round-trips.
