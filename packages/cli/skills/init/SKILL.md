---
name: st-init
description: "Software Teams: Initialise Software Teams state and hooks in the current project"
...
Resolve the CLI per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`. If resolution fails the
...
natively — project skill copies and `.claude/agents/` files. It **does**
...
> Software Teams initialised. Run `/st-create-plan` to plan your next feature,
> or `/st-implement-plan` to start building.
>
> Plugin skills use the common bare `/st-*` aliases (or the explicit
> `/software-teams:*` namespace); no project skill copies are generated in
> plugin mode.
