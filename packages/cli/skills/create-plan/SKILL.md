...
> **Why source matters:** Software Teams specialists (`software-teams-backend`, `software-teams-frontend`, etc.) are converted from `framework/agents/` into `.claude/agents/` by `$ST_CLI sync-agents` (resolve per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`), so once that has run they are spawned natively by name (`subagent_type="software-teams-backend"`). The `source:` field still distinguishes specialists shipped with Software Teams (`source: software-teams`) from user-added Claude Code subagents (`source: claude-code`) for routing decisions. See `framework/software-teams.md` Critical Constraints and `framework/components/meta/AgentRouter.md` §4.
...
Resolve the CLI per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`. Run the state CLI — do NOT manually edit `.software-teams/state.yaml`. The `--plan-path` argument is the **canonical index** for the chosen tier:
...
6. **Deterministic / unattended run (three-tier plans only)** — one line, include ONLY when the plan resolved to three-tier: `` Once approved: run it deterministically with `/st-compile-workflow` (or `/st-implement-plan --workflow`); for a large plan, run it unattended with `/goal` and be pinged at blockers/completion. `` Omit this line entirely for single-tier plans.
