---
name: st-compile-workflow
description: "Software Teams: Compile an approved three-tier plan into a deterministic Workflow only on an explicit compile or workflow request"
when_to_use: "Only when the current user message explicitly asks to compile or run a deterministic workflow; never infer this from plan approval."
...
separate terminal. Before continuing, require the current user message to
explicitly request workflow compilation/execution (or directly invoke this
skill). Plan approval alone is not permission to compile or run anything.
This is the opt-in deterministic alternative to
...
Resolve the CLI per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`. If resolution fails,
