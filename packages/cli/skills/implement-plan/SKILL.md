---
name: st-implement-plan
description: "Software Teams: Execute an approved implementation plan only when the user explicitly asks to implement, build, or execute it"
when_to_use: "Only when the current user message explicitly requests implementation. Plan approval or refinement alone must never trigger this skill."
...
Inspect the current user message before reading or changing anything. Continue
only if that message explicitly asks to **implement**, **build**, or **execute**
the approved plan (or directly invokes this skill). Approval phrases such as
"approved", "looks good", or "what's next?" are not implementation requests.
On approval alone, say the plan is locked in and wait. Do not invoke another
skill and do not advance state.
...
Resolve the CLI per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`, then run `$ST_CLI state executing`. Do NOT manually edit `.software-teams/state.yaml`.
...
2. **Spawn the pinned agent natively.** Use `subagent_type="{task.agent}"` (post-T6 native migration — Claude Code resolves the spec from `.claude/agents/{task.agent}.md` directly). Spawn under `mode: "acceptEdits"` with the scoped allowlist from `.claude/settings.json`. The prompt MUST include:
   ...
   **Register the spawn (lifecycle tracking).** Before each spawn, open a ledger entry so a stalled agent can be reaped (see § Lead monitoring) and so future plans get real per-spawn cost numbers:
   ...
   This opens an entry in `.software-teams/persistence/spawn-ledger.jsonl` with a 30-minute deadline. `$ST_CLI spawn-log report` (resolve per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`) produces aggregate cost numbers; `$ST_CLI spawn-log reap` lists entries that blew their deadline. The matching `spawn-log complete` call in step 3 closes the entry.
...
5. **Branch on verify result:**
   - **Pass** → run `$ST_CLI state advance-task {task-id}` (resolve per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`), record verification in the task summary, continue to next task.
...
- All agents (both `source: software-teams` and `source: claude-code`) are spawned natively by name: `subagent_type="{task.agent}"`. Claude Code resolves the spec from `.claude/agents/{task.agent}.md`. Software Teams specialists land there via `$ST_CLI sync-agents` (resolve per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`; this converts `framework/agents/software-teams-*.md` to the Claude Code-compatible format). The legacy `subagent_type="general-purpose"` + identity-injection pattern is documented as a fallback only — see `framework/components/meta/AgentRouter.md` §4 and `framework/software-teams.md` Critical Constraints.
<!-- /lint-allow -->
...
- The variable `{plan.primary_agent}` resolves to the native agent name (e.g. `software-teams-backend`, `software-teams-frontend`, `unity-specialist`) for both `source: software-teams` and `source: claude-code` after `$ST_CLI sync-agents` has run (resolve per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`).
...
After each task's programmer returns successfully, resolve the CLI per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`, then run:
...
Resolve the CLI per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`, then run `$ST_CLI state complete`. Do NOT manually edit `.software-teams/state.yaml`.
