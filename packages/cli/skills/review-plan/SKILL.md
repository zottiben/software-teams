...
Resolve the CLI per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`. Record via the state CLI (NEVER hand-edit `state.yaml`):
...
**`one_shot_ready: true` →** AUTO-APPROVE: run `$ST_CLI state approved` (resolve per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`). Then print exactly:
...
| `$ST_CLI state` CLI missing or errors (resolve per `${CLAUDE_SKILL_DIR}/../st-support/cli-invocation.md`) | Run `$ST_CLI init` to initialise the state machine, then retry. |
